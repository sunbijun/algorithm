...
public class KdTree {
    ...
    public           Point2D nearest(Point2D p){
        ...
        if (isEmpty())
            return null;
        Node nearestP = root;
        nearestP = getNearest(root,nearestP, p);
        return nearestP.p;
    }

    private Node getNearest(Node cursor, Node nearestP, Point2D p){
        if (cursor == null){
            return nearestP;
        }
        else{
            if(GetDistanceToRect(cursor, p) > nearestP.p.distanceTo(p)){
                ...
                Node nearestP1 = nearestP;
                if(cursor.p.distanceTo(p) < nearestP.p.distanceTo(p))
                        nearestP1 = cursor;
                Comparator<Point2D> cmp = cmp(cursor);
                if(cmp.compare(p,cursor.p) < 0){
                    nearestP1 = getNearest(cursor.lb,nearestP1, p);
                    nearestP1 = getNearest(cursor.rt,nearestP1, p);
